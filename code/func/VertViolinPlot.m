...
% This function creates vertical violin plots for visualizing data distributions
% and densities across different groups. It's especially useful for comparative
% data analysis, displaying the spread and density of data points. Optionally,
% individual data points can be plotted over the violin plots.

% Author:
% S.H. Castañón


[Handles_all] = VertViolinPlot(X,Y,Group,ColZ, BndWdth,DoDots)
if nargin < 2
    Y = 1;
end
if nargin < 3
...
for i_col = 1:size(Y,2)
   ...
   for i_gr = 1:num_gr
       ...
       if DoDots
           ...
           %plot(X(1,i_col)+scatter_x,scatter_y,'.','color',ColZ(i_gr,:).*.5,'MarkerSize',10)%NK added marker size
           plot(X(1,i_col)+scatter_x,scatter_y,'.','color',ColZ(i_gr,:),'MarkerSize',15)%NK added marker size
       end
       %alpha(0.5)
