...
%                               e.g. [1,1,2] = subjects 1&2 belongi to group 1,
%                               subject 3 belongis to group 2.
...
%% ---------- Input data ----------
 clear all ; clc;
load('phen.mat')
phen_n=phen.Properties.VariableNames;

phen=table2array(phen);

%remove rows containing missing phenotype data

phen=phen(logical(abs(1-any(isnan(phen),2))),:);

%remove the age effect from the data
phen_r=regress_out(phen(:,2:end),phen(:,1),[]);
% 
phen=[phen(:,1),phen_r];

clearvars -except phen phen_n outputpath


for i =1:size(phen,2)
    
    eval([phen_n{i} '=  phen(:,i)']);
end



outputpath =fullfile(filesep,'Volumes','CODE_development','Nada','PROXIMITY','GIT_HUB_PROXIMITY')

mkdir(outputpath);

%% variables

data=PI_NORM51;% Matrix X0 is typically a matrix with brain imaging data,of size subjects (rows) x imaging features (columns)
dxcov=ones(size(data,1),1);% of size subjects (rows) grouping or diagnosis



Y0behav=[ADOS_SA,ADOS_RRB,PEP_CVP_DQ,PEP_IOM_DQ,PEP_LE_DQ,...
    PEP_LR_DQ,PEP_MF_DQ,PEP_MG_DQ,VABS_COM,VABS_DAI,VABS_SOC,VABS_MOT ];% 
...
input.grouping = dxcov;
% --- Names of the groups ---
% here you can specify the names of the groups for the plots
input.group_names={''}; %% the first name is the smallest number in your groups

% --- Names of the behavior data ---

input.behav_names = {'ADOS SA','ADOS RRB','PEP CVP','PEP IOM','PEP LE',...
    'PEP LR','PEP MF','PEP MG', 'VABS COM','VABS DAI','VABS SOC','VABS MOT' }
...
save_opts.output_path = outputpath;
...
input.brain_data = input.brain_data(:,1); 
% save_opts.img_type = 'barPlot';
